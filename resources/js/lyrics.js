...
function setupProviderUI() {
    ...
    if (modal) {
        ...
        // FIX: Prevent pull-to-refresh in Fully Kiosk Browser
        // Add touch handler directly on modal content to prevent pull-to-refresh when at top
        ...
        if (modalContent) {
            ...
            // CRITICAL: { passive: false } is required for preventDefault() to work
            // This prevents pull-to-refresh when scrolled to top and pulling down
            modalContent.addEventListener('touchmove', (e) => {
                ...
                // Only prevent if at top AND pulling down (pull-to-refresh gesture)
                if (isAtTop && isPullingDown) {
                    e.preventDefault();
                    e.stopPropagation();
                }
            }, { passive: false }); // CRITICAL: Must be false for preventDefault to work
        }
        
        // Also handle scrollable areas inside modal (provider-list and album-art-grid)
        // These might be the actual scrollable elements on desktop view
        const providerList = document.getElementById('provider-list');
        const albumArtGrid = document.getElementById('album-art-grid');
        
        [providerList, albumArtGrid].forEach(scrollableElement => {
            if (!scrollableElement) return;
            
            let elementTouchStartY = 0;
            
            scrollableElement.addEventListener('touchstart', (e) => {
                if (e.touches.length > 0) {
                    elementTouchStartY = e.touches[0].clientY;
                }
            }, { passive: true });
            ...
            scrollableElement.addEventListener('touchmove', (e) => {
                ...
                const currentY = e.touches[0].clientY;
                const deltaY = currentY - elementTouchStartY;
